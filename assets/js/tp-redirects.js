(function ($) {
  var TP_Redirects = function (el) {
    ...
    this.events = function () {
      ...
      $(el).on("keyup", "input#tp-redirects-search", function (event) {
        ...
        if (term != $(this).val()) $(this).val(term);
        ...
        if (8 == event.which) self.waiting = false; //Always listen to backspace

        if (term == self.prevTerm && 0 == term.length) return;

        if (2 < term.length) self.search(term);
        else if (0 == term.length) self.search("");
      ...
      $(el).on(
        "keyup",
        "td.source input, td.destination input",
        function (event) {
          if (13 == event.which) {
            self.save(
              $(this).closest("tr").data("source"),
              $(this).closest("tr").find("td.source input").val(),
              $(this).closest("tr").find("td.destination input").val(),
              function () {
      ...
      $(window).scroll(function () {
        if (
          "" == $(el).find("#tp-redirects-search").val() &&
    ...
    this.search = function (term) {
      if (0 < term.length) self.point("tp-redirects-search");
      else self.point("tp-redirects-search", "close");
    ...
    this.catchup = function () {
      if (
        !self.tryLater ||
        $(el).find("input#tp-redirects-search").val() != self.tryLater
      ) {
        self.tryLater = "";
        return;
      }
    ...
    this.parse = function (response) {
      if (response.html) {
        ...
        else $(el).find("table.tp-redirects-table tbody").append(response.html);
      } else if (response.replace) {
        $(el)
          .find("table.tp-redirects-table tbody")
          .html(
            '<tr><td colspan="3">' +
              TP_Redirects_Labels["not_found"] +
              "</td></tr>"
          );
      }

      if (response.page) self.page = response.page;
    ...
    this.edit = function (source, focus) {
      ...
      row
        .find("td.actions")
        .html(
          '<a class="dashicons dashicons-yes tp-redirects-edit-finish" title="' +
            TP_Redirects_Labels["edit_finish"] +
            '"></a>'
        );
      row
        .find("td.actions")
        .append(
          '<a class="dashicons dashicons-no-alt tp-redirects-edit-dismiss" title="' +
            TP_Redirects_Labels["edit_dismiss"] +
            '"></a>'
        );

      if ("destination" == focus) row.find("td.destination input").focus();
      else if ("source" == focus) row.find("td.source input").focus();
    };
    ...
    this.remove = function (source) {
      if (!confirm(TP_Redirects_Labels["delete_confirm"])) return;
    ...
    this.point = function (to, action, dismiss) {
      ...
      if (TP_Redirects_Pointers[to]) {
        if (closedPointers[to]) return;

        var pointer = TP_Redirects_Pointers[to];

        var pointer_el = $(pointer["element"])
          .pointer({
            content:
              "<h3>" +
              pointer["header"] +
            ...
            close: function () {
              ...
              if (action == "close" && !dismiss) closedPointers[to] = false;
  ...
  $.fn.tp_redirects = function () {
    return this.each(function () {
      if ($(this).data("tp_redirects")) return;
